...
class SArray {
 public:
  ...
  friend BinStream& operator<<(BinStream& bin, const SArray<V>& sarray) {
    bin << sarray.size();
    bin.push_back_bytes(reinterpret_cast<char*>(sarray.data()), sarray.size()*sizeof(V));
    return bin;
  }
  friend BinStream& operator>>(BinStream& bin, SArray<V>& sarray) {
    size_t size;
    bin >> size;
    void* ptr = bin.pop_front_bytes(size*sizeof(V));
    sarray.CopyFrom(static_cast<V*>(ptr), size);
    return bin;
  }
